...
Files_Get_Module_Files() {
  files=$(find $(Files_Path_Root) -name "*.sh" -not -name "*.test.sh" -not -name "*.local.sh" | LC_COLLATE=C sort)
...
Files_Get_Test_Files() {
  files=$(find $(Files_Path_Root) -path "*/*.test.sh" -or -path "*/*.test" | LC_COLLATE=C sort)
